...
if (import.meta.url === pathToFileURL(path.resolve(process.argv[1] ?? '')).href) {
  ...
  const clientRoot =
    ...
    [path.resolve(webRoot, 'node_modules/duneta/client'), path.resolve(webRoot, 'node_modules/duneta-client')].find(
      (dir) => fs.existsSync(path.join(dir, 'package.json')),
    );

  if (!clientRoot) {
    throw new Error('[duneta] duneta/client not found. Install duneta or duneta-client.');
  }
